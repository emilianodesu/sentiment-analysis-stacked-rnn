...
|---|---------------|---------------|-----------|-----------------|
| 1 | `model.py` | 27-62 | Clase `SentimentRNN.__init__` — definición de capas (embedding, RNN, fc) | arquitectura.tex |
| 2 | `model.py` | 64-75 | Inicialización del bias de forget gate a 1.0 | arquitectura.tex |
| 3 | `model.py` | 77-99 | Método `forward` — flujo de datos embedding → RNN → fc | arquitectura.tex |
| 4 | `train.py` | 108-120 | Configuración de criterion (BCEWithLogitsLoss) y optimizer (Adam) | metodologia.tex |
| 5 | `train.py` | 143-163 | Bucle de entrenamiento: forward, backward, gradient monitoring | metodologia.tex |
| 6 | `train.py` | 165-168 | Gradient clipping (`clip_grad_norm_`) | metodologia.tex |
| 7 | `train.py` | 192-210 | Early stopping: comparación val_loss, guardado de checkpoint | metodologia.tex |
| 8 | `data.py` | 12-30 | Función `clean_text` — preprocesamiento de texto | metodologia.tex |
| 9 | `data.py` | 68-78 | Función `pad_sequence` — truncamiento y padding | metodologia.tex |
| 10 | `evaluate.py` | 55-80 | Inferencia: carga de checkpoint, forward sin gradientes, sigmoid > 0.5 | resultados.tex |
